# ============================================================
# Script: 02_season_DE_15days.R
# Purpose: Differential expression analysis between winter-spring
#          and summer-autumn using the Nov 15 cutoff. Tests main
#          season effect, asthma effect, and models corrected for
#          ciliated cell proportions. Produces volcano plots.
# Input:   ATLANTIS_master_table_seasons_new.csv
#          ATLANTIS_raw_readcount_dedup_FINAL.csv
#          CIBERSORT.proportions.csv
# Output:  DE_genes_15Nov_winter_spring.csv
#          DE_genes_15Nov_asthma.csv
#          DE_genes_15Nov_asthma_season_ciliated_cor.csv
#          plots/Volcano_seasons_15Nov.png
# ============================================================
...
source(file.path(conf$script_path, "R/utils_DE.R"))
...
edger <- run_edger_pipeline(expression.data, design)
...
#                                  levels = design)
qlf_seasons <- edgeR::glmQLFTest(edger$fit, coef = 2)
...
qlf_asthma <- edgeR::glmQLFTest(edger$fit, coef = 7)
...
gene_ids <- rownames(edger$DGEL$counts)
...
de_result_asthma <-  topTags(qlf_asthma, n = nrow(edger$DGEL))$table %>%
  tibble::rownames_to_column("Gene") %>%
  left_join(all_new_gene, by = c("Gene" = "ensembl_gene_id"))
...
qlf_seasons_ciliated_corrected <- edgeR::glmQLFTest(edger$fit, coef = 2)
...
qlf_ciliated <- edgeR::glmQLFTest(edger$fit, coef = 8)
...
qlf_asthma_season_ciliated_corrected <- edgeR::glmQLFTest(edger$fit, coef = 7)
...
de_result_asthma_season_ciliated_corrected <- topTags(qlf_asthma_season_ciliated_corrected, n = nrow(edger$DGEL))$table %>%
  tibble::rownames_to_column("Gene") %>%
  left_join(all_new_gene, by = c("Gene" = "ensembl_gene_id")) %>%
...
qlf_asthma_ciliated_corrected <- edgeR::glmQLFTest(edger$fit, coef = 7)
...
de_result_asthma_ciliated_corrected <- topTags(qlf_asthma_ciliated_corrected, n = nrow(edger$DGEL))$table %>%
  tibble::rownames_to_column("Gene") %>%
  left_join(all_new_gene, by = c("Gene" = "ensembl_gene_id")) %>%
