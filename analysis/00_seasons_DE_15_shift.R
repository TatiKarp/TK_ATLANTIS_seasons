# ============================================================
# Script: 00_seasons_DE_15_shift.R
# Purpose: Identify the optimal seasonal boundary by sliding a
#          15-day window across the calendar year and counting
#          differentially expressed genes at each position.
#          Produces a circular plot of DEG count vs. date.
# Input:   ATLANTIS_master_table_seasons_new.csv
#          ATLANTIS_raw_readcount_dedup_FINAL.csv
# Output:  15days_shift.Rdata
#          plots/circled_date_genes.png
# ============================================================
...
for (i in 1:(length(breaks_df)/2)) {
  ...
  contrasts <- limma::makeContrasts(season_binary = groupsgroup1 - groupsgroup2,
                                    levels = design)
  qlf <- edgeR::glmQLFTest(edger$fit, contrast = contrasts[,"season_binary"])
